...
#copied from other script i know ... could extrac to common or so but is that needed?
from datasets import load_dataset, Dataset, DatasetDict
import os
from PIL import Image

def load_image_text_dataset(data_dir_train, data_dir_val=None):
    dict_dataset = {}
    
    if data_dir_train is not None:
        data = {"image": [], "text": []}
        for filename in os.listdir(data_dir_train):
            if filename.endswith(".png"):
                img_path = os.path.join(data_dir_train, filename)
                txt_path = os.path.join(data_dir_train, filename.replace(".png", ".txt"))
        ...
        dict_dataset = {"train": Dataset.from_dict(data)}
    else:
        raise ValueError("data_dir_train is required")
     
        
    if data_dir_val is not None:
        data = {"image": [], "text": []}
        for filename in os.listdir(data_dir_val):
            if filename.endswith(".png"):
                img_path = os.path.join(data_dir_val, filename)
                txt_path = os.path.join(data_dir_val, filename.replace(".png", ".txt"))
                ...
                if os.path.exists(txt_path):
                    ...
                    data["image"].append(image)
                    data["text"].append(text)
        dict_dataset["val"] = Dataset.from_dict(data)
